...
- **Windows 切 Ubuntu 后停在 GRUB 菜单**：脚本已用一次性标记文件方案解决——`to-ubuntu.ps1` 会在 EFI 分区根目录写入 `bootubuntu.once`；GRUB 经 `/etc/grub.d/40_custom` 检测到后本次直接启动 Ubuntu（只读不删）；Ubuntu 开机后由 `bootflag-clean.service` 删除标记。两个配置文件见 `setup/` 目录。手动开机的默认项（Windows、10 秒倒计时）不受影响。
